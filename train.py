...
class Dataset:
    def __init__(
            self, cell_gene_file, cell_exp_file, test_file, train_test_split,
            hvg_file=None, cell_meta_file=None, source=None,
    ):
        ...
        for cell, (gene_seq, exp_seq) in enumerate(zip(cell_gene_data, cell_exp_data)):
            ...
            if cell in testcell_to_maskseq:
                ...
                self.test_data.append((cell, gene_seq, exp_seq, mask_seq, hvgmask_seq))

                if train_test_split == "merge":
                    self.train_data.append((cell, gene_seq, exp_seq))
                else:
                    assert train_test_split == "split"
            else:
                self.train_data.append((cell, gene_seq, exp_seq))
        ...
        if cell_meta_file and source:
            cell_meta_data = read_csv(cell_meta_file, "csv")
            cell_meta_data = cell_meta_data[1:]
            cell_list = []
            for ci, (_cell, _total_count, _expressed_genes, cell_source) in enumerate(cell_meta_data):
                if cell_source not in source:
                    continue
                cell_list.append(ci)
                self.cell_set = set(cell_list)
        else:
            self.cell_set = set(range(len(cell_gene_data)))
        source_cells = len(self.cell_set)
        logger.info(f"{source_cells:,} {source} cells")
        return
    ...
    def create_train_batch(self, batch_size):
        ...
        data = [
            (gene_seq, exp_seq)
            for cell, gene_seq, exp_seq in data
    ...
    def create_test_batch(self, batch_size):
        ...
        data = [
            (gene_seq, exp_seq, mask_seq, hvgmask_seq)
            for cell, gene_seq, exp_seq, mask_seq, hvgmask_seq in data
            if cell in self.cell_set
        ]
        cells = len(data)
        batch_list = []
    ...
    def create_weight_batch(self):
        ...
        for cell, gene_seq, exp_seq in data:
            ...
            cell_batch_list.append(
                (cell, [[gene_seq, exp_seq, mask_seq, hvgmask_seq]])
            )

        return cell_batch_list
...
def run_train(arg):
    ...
    dataset = Dataset(
        arg.cell_gene_file, arg.cell_exp_file, arg.test_file, arg.train_test_split,
        hvg_file=arg.hvg_file, cell_meta_file=arg.cell_meta_file, source=arg.source,
    )
...
def run_weight(arg):
    ...
    cell_batch_list = dataset.create_weight_batch()
    batches = len(cell_batch_list)
    logger.info(f"{batches:,} batches")

    # gene-gene matrices
    os.makedirs(arg.weight_dir, exist_ok=False)
    with torch.no_grad():
        for bi, (cell, batch) in enumerate(cell_batch_list):
            (
            ...
            cell += 1
            file = os.path.join(arg.weight_dir, f"{cell}.npy")
            np.save(file, w)
...
def main():
    ...
    parser.add_argument("--hvg_file", type=str, default=None)

    parser.add_argument("--cell_meta_file", type=str, default=None)
    parser.add_argument("--source", type=str, default=None)

    parser.add_argument("--test_file", type=str, default="train/test.json")
    parser.add_argument("--train_test_split", type=str, choices=["split", "merge"])
