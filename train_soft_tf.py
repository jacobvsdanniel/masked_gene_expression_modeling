...
class Dataset:
    def __init__(self, cell_gene_file, cell_exp_file, test_file, hvg_file=None, tf_file=None):
        ...
        for cell, (gene_seq, exp_seq) in enumerate(zip(cell_gene_data, cell_exp_data)):
            ...
            tfmask_seq = [
                1 if self.tf_set and gene in self.tf_set else 0
...
def run_train(arg):
    ...
    if arg.model.startswith("model_transformer__"):
        _, layers, dimension, heads, dimension_feedforward, initial_tf_weight, train_tf_weight = arg.model.split("__")

        layers = int(layers)
        dimension = int(dimension)
        heads = int(heads)
        dimension_feedforward = int(dimension_feedforward)
        initial_tf_weight = float(initial_tf_weight)
        train_tf_weight = train_tf_weight == "T"

        model = ModelTransformer(
            layers, dimension, heads, dimension_feedforward,
            initial_tf_weight, train_tf_weight,
        )
        tf_weight = model.get_tf_weight()
        logger.info(f"[model init] tf_weight={tf_weight:.2e}")
    else:
        assert False
    model.to("cuda")
    model.train()
